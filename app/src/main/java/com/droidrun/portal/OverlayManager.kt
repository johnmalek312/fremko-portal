    ...
    inner class OverlayView(context: Context) : FrameLayout(context) {
        ...
        private fun drawElement(canvas: Canvas, elementInfo: ElementInfo) {
            try {
                ...
                // Draw a rounded rectangle border for a sleeker look
                val radius = 10f
                canvas.drawRoundRect(
                    elementInfo.rect.left.toFloat(),
                    elementInfo.rect.top.toFloat(),
                    elementInfo.rect.right.toFloat(),
                    elementInfo.rect.bottom.toFloat(),
                    radius,
                    radius,
                    boxPaint
                )
                ...
                // Calculate background rounded rectangle for the text badge
                val backgroundPadding = 6f
                val badgeLeft = (textX - backgroundPadding)
                val badgeTop = (textY - textHeight)
                val badgeRight = (textX + textWidth + backgroundPadding)
                val badgeBottom = (textY + backgroundPadding)
                canvas.drawRoundRect(
                    badgeLeft,
                    badgeTop,
                    badgeRight,
